      ...
      <li v-for="value in scripts" :key="value.id" style="width: 70%;">
        ...
        >
          ...
          <button
            ...
            v-show="value.author === userName"
            @click="DeletePost(value.slug, value)"
          >
            Delete
          </button>
        </b-card>
...
export default {
  ...
  methods: {
    ...
    DeletePost: function(POST, value) {
      // axios.delete('/user', {data: {userId: 'xxxx'}}).then(res => {
      // })
      if (POST != null) {
        this.delete_slug = POST;
        let delete_url = this.url + String(POST) + "/";
        if (confirm("Are you sure to delete '" + value.content + "'?")) {
          axios({
            method: "DELETE",
            url: delete_url,
            headers: {
          ...
          // 指定要素の番号取得
          let index = this.scripts.indexOf(value);
          // その番号から削除する要素数をカット
